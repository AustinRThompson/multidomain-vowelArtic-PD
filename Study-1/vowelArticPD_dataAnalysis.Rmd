---
title: "A multidimensional study of articulation deficits in Parkinson’s disease - Data Analysis: Study 1"
---
...
```{r}
# Building the model
Int_m0 <- robustlmm::glmmTMB(Int ~ 1 +
                             (1 | ListenerID) +
                             (1 | StudyID),
...
```{r}
# Building the model
Int_m1 <- robustlmm::rlmer(Int ~ Group * condition +
                             (1 | ListenerID) +
                             (1 | StudyID),
                           data = modelData)
summary(Int_m1)
sjPlot::tab_model(Int_m1)
...
base::saveRDS(
  ...
  file = "Models/Int_finalModel.rds")
...
```{r}
# Building the model
aVSA_m0 <- lmerTest::lmer(log(aVSA) ~ 1 +
                       (1 | StudyID),
                     data = modelData)
...
```{r}
# Building the model
aVSA_m1 <- lmerTest::lmer(log(aVSA) ~ condition +
                       (1 | StudyID),
                     data = modelData)
# Model summary
summary(aVSA_m1)
...
```{r}
# Building the model
aVSA_m2 <- lmerTest::lmer(log(aVSA) ~ condition*Group +
                       (1 | StudyID),
                     data = modelData)
# Model summary
summary(aVSA_m2)
...
base::saveRDS(
  ...
  file = "Models/aVSA_finalModel.rds")
...
```{r}
# Building the model
acoDistance_m0 <- glmmTMB::glmmTMB(acoDistance ~ 1 +
                                     (1 | StudyID),
...
```{r}
# Building the model
acoDistance_m1 <- lmerTest::lmer(log(acoDistance) ~ Group + condition +
                                     (1 | StudyID),
                         data = modelData)
# Model summary
summary(acoDistance_m1)
anova(acoDistance_m1)
...
```{r}
# Building the model
acoDistance_m2 <- glmmTMB::glmmTMB(
  acoDistance ~ Group + Sex +
    (1 | StudyID),
  family = Gamma(link = "log"),
  data = modelData
)
...
base::saveRDS(
  ...
  file = "Models/acoDistance_finalModel.rds")
...
```{r}
# Building the model
F2_Slope_m0 <- lmerTest::lmer(log(F2_Slope) ~ 1 +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(F2_Slope_m0)
...
```{r}
# Building the model
F2_Slope_m1 <- lmerTest::lmer(log(F2_Slope) ~ condition +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(F2_Slope_m1)
...
```{r}
# Building the model
F2_Slope_m2 <- lmerTest::lmer(log(F2_Slope) ~ condition*Group +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(F2_Slope_m2)
...
```{r}
# Building the model
F2_Slope_m3 <- lmerTest::lmer(log(F2_Slope) ~ condition*Group*Sex +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(F2_Slope_m3)
...
base::saveRDS(
  ...
  file = "Models/F2_Slope_finalModel.rds")
...
```{r}
# Building the model
kVSA_m1 <- stats::lm(log(kVSA) ~ condition,
                         data = modelData)
# Model summary
summary(kVSA_m1)
...
```{r}
# Building the model
kVSA_m2 <- stats::lm(log(kVSA) ~ condition*Group,
                         data = modelData)
# Model summary
summary(kVSA_m2)
...
```{r}
# Building the model
kVSA_m3 <- lmerTest::lmer(log(kVSA) ~ condition*Group*Sex +
                            (1|StudyID),
                         data = modelData)
# Model summary
summary(kVSA_m3)
...
base::saveRDS(
  ...
  file = "Models/kVSA_finalModel.rds")
...
```{r}
# Building the model
kinDistance_m0 <- lmerTest::lmer(log(kinDistance) ~ 1 +
                           (1 | StudyID),
                         data = modelData)
# Model summary
...
```{r}
# Building the model
kinDistance_m1 <- lmerTest::lmer(log(kinDistance) ~ condition +
                           (1 | StudyID),
                         data = modelData)
# Model summary
...
```{r}
# Building the model
kinDistance_m2 <- lmerTest::lmer(log(kinDistance) ~ condition*Group + Int_M +
                           (1 | StudyID),
                         data = modelData)
# Model summary
...
```{r}
# Building the model
kinDistance_m3 <- lmerTest::lmer(log(kinDistance) ~ condition*Group*Sex +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(kinDistance_m3)
anova(kinDistance_m3)
...
base::saveRDS(
  ...
  file = "Models/kinDistance_finalModel.rds")
...
```{r}
# Building the model
TB_speedMax_m0 <- lmerTest::lmer(log(kinSpeed) ~ 1 +
                           (1 | StudyID),
                         data = modelData)
# Model summary
...
```{r}
# Building the model
TB_speedMax_m1 <- lmerTest::lmer(log(kinSpeed) ~ condition +
                           (1 | StudyID),
                         data = modelData)
# Model summary
...
```{r}
# Building the model
TB_speedMax_m2 <- lmerTest::lmer(log(kinSpeed) ~ condition*Group +
                           (1 | StudyID),
                         data = modelData)
# Model summary
...
base::saveRDS(
  ...
  file = "Models/TB_speedMax_finalModel.rds")
  ...
  dplyr::mutate(Sex = factor(Sex, levels = c("M","F")),
                condition = factor(condition,
                                   levels = c("conv",
                                              ...
                                              "lessClear")))
modelData <- aiMeasures
hist(modelData$Jaw_distance)
performance::check_distribution(modelData$Jaw_distance)
...
```{r}
# Building the model
kinDistance_m1 <- lmerTest::lmer(Jaw_distance ~ condition +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(kinDistance_m1)
...
```{r}
# Building the model
kinDistance_m2 <- lmerTest::lmer(Jaw_distance ~ condition*Group +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(kinDistance_m2)
anova(kinDistance_m2)
...
```{r}
# Building the model
kinDistance_m3 <- lmerTest::lmer(Jaw_distance ~ condition*Group*Sex +
                           (1 | StudyID),
                         data = modelData)
# Model summary
summary(kinDistance_m3)
