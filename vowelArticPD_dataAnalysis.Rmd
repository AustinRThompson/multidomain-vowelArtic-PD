...
table_group <- summary(pairs(emms_group)) %>%
    dplyr::mutate(
    sig = ifelse(p.value < alpha_s1, TRUE, FALSE),
    apa = paste0(Sex, ", ",
                 condition, ", ",
                 contrast, ": z = ",
                 round(z.ratio, digits = 2), ", ",
                 ifelse(p.value < .001, "p < .001", paste0"p = ",round(p.value, digits = 3)))))
table_groupCondition <- summary(pairs(emms_condition)) %>%
  dplyr::mutate(
    sig = ifelse(p.value < alpha_s1, TRUE, FALSE),
    apa = paste0(Sex, ", ",
                             Group, ", ",
                             contrast, ": z = ",
                             round(z.ratio, digits = 2), ", ",
                             ifelse(p.value < .001, "p < .001", round(p.value, digits = 3))))
...
predictions  <- stats::predict(aVSA_finalModel, se = T)
plotData$mpg   <- predictions$fit
...
aVSA_finalModel <- lmerTest::lmer(aVSA ~ condition*Group*Sex +
                       (1 | StudyID),
                     data = modelData %>%
...
plots_aVSA <- sjPlot::plot_model(aVSA_finalModel,
                   type = "int",
                   show.values = TRUE)

plot_aVSA <- plots_aVSA[[4]]$data %>%
  as.data.frame() %>%
  dplyr::mutate(
    condition = factor(x,
                       levels = c(3, 1, 2),
...
# Compare EMMs for Group and Sex
table_group_aVSA <- summary(pairs(emms_group)) %>%
    dplyr::mutate(
    sig = ifelse(p.value < alpha_s1, TRUE, FALSE),
    apa = paste0(Sex, ", ",
                 condition, ", ",
                 contrast, ": t = ",
                 round(t.ratio, digits = 2), ", ",
                 ifelse(p.value < .001, "p < .001", round(p.value, digits = 3))))
table_groupCondition_aVSA <- summary(pairs(emms_condition)) %>%
  dplyr::mutate(
    sig = ifelse(p.value < alpha_s1, TRUE, FALSE),
    apa = paste0(Sex, ", ",
                             Group, ", ",
...
## 1. Intelligibility
```{r}
model <- base::readRDS("Data/Models/Study 1/Int_finalModel.rds") %>%
...
## 2. Articulatory Precision
```{r}
model <- base::readRDS("Data/Models/Study 1/AP_finalModel.rds") %>%
  sjPlot::plot_model(type = "int")
...
## 3. Acoustic VSA
```{r}
model <- base::readRDS("Data/Models/Study 1/aVSA_finalModel.rds") %>%
  sjPlot::plot_model(type = "int")
...
## 4. Acoustic Distance
```{r}
model <- base::readRDS("Data/Models/Study 1/acoDistance_finalModel.rds") %>%
  sjPlot::plot_model(type = "int")
...
## 5. F2 Slope
```{r}
model <- base::readRDS("Data/Models/Study 1/F2_Slope_finalModel.rds") %>%
  sjPlot::plot_model(type = "int")
...
## 7. Kin Distance
```{r}
model <- base::readRDS("Data/Models/Study 1/kinDistance_finalModel.rds") %>%
  sjPlot::plot_model(type = "int")
...
## 8. TB Speed
```{r}
model <- base::readRDS("Data/Models/Study 1/TB_speedMax_finalModel.rds") %>%
  sjPlot::plot_model(type = "int")
