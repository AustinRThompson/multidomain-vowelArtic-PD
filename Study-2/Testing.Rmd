---
title: "A multidimensional study of articulation deficits in Parkinson’s disease - Data Analysis: Study 2"
---
...
aiMeasures <- rio::import(file = "../Data/PreppedData/CollatedData/TargetMeasures_aiMeasures.csv") %>%
  dplyr::mutate(Sex = factor(Sex, levels = c("M","F")),
                kinSpeed = TB_displacement/(duration*1000))
```
...
# Checking the distribution and normality of the target variables
## Intelligibility
  hist(modelData$Int_M) # Looks left-skewed
  performance::check_normality(modelData$Int_M) # Non-normality detected
  performance::check_distribution(modelData$Int_M)
  
## Articulatory Precision
  hist(modelData$AP_M) # Looks left-skewed
  performance::check_normality(modelData$AP_M) # Non-normality detected
  performance::check_distribution(modelData$AP_M)
  
## aVSA
  hist(modelData$aVSA) # Looks right-skewed
  performance::check_normality(modelData$aVSA) # Non-normality detected
  performance::check_distribution(modelData$aVSA)
  
## kVSA
  hist(modelData$kVSA) # Looks right-skewed
  performance::check_normality(modelData$kVSA) # Non-normality detected
  performance::check_distribution(modelData$kVSA)
  
# Creating log transformed variables
modelData <- modelData %>%
  dplyr::mutate(Int_M_log = log(Int_M),
                Int_M_cube = Int_M^(1/3),
                AP_M_log = log(AP_M),
                aVSA_log = log(aVSA),
                kVSA_log = log(kVSA)
                )

hist(modelData$Int_M)
hist(modelData$Int_M_log)
hist(modelData$Int_M_cube)

# Load the MASS package
library(MASS)

# Apply Box-Cox transformation
boxcox_result <- boxcox(modelData$Int_M ~ 1, lambda = seq(-5, 5, 0.1))

# Find the best lambda
best_lambda <- boxcox_result$x[which.max(boxcox_result$y)]

# Transform the data using the best lambda
boxcox_data <- if (best_lambda == 0) log(modelData$Int_M) else (modelData$Int_M^best_lambda - 1) / best_lambda

# Plot histogram of Box-Cox transformed data
hist(boxcox_data, main="Histogram of Box-Cox Transformed Data", xlab="Box-Cox Transformed modelData$Int_M", breaks=20)

# Q-Q plot for Box-Cox transformed data
qqnorm(boxcox_data, main="Q-Q Plot of Box-Cox Transformed Data")
qqline(boxcox_data, col="red")
...
Int_VSA_comp <- robustlmm::rlmer(Int_M ~ 
                                 aVSA*Sex +
                                 kVSA*Sex +
                               (1 | SpeakerID),
                             data = modelData)
summary(Int_VSA_comp)
sjPlot::tab_model(Int_VSA_comp)

Int_VSA_pars <- robustlmm::rlmer(Int_M ~ 
                                 aVSA*Sex +
                               (1 | SpeakerID),
                             data = modelData)
summary(Int_VSA_pars)
sjPlot::tab_model(Int_VSA_pars)

## AP

AP_VSA_comp <- robustlmm::rlmer(AP_M ~ 
                                 aVSA*Sex +
                                 kVSA*Sex +
                               (1 | SpeakerID),
                             data = modelData)
summary(AP_VSA_comp)
sjPlot::tab_model(AP_VSA_comp)

AP_VSA_pars <- robustlmm::rlmer(AP_M ~ 
                                 aVSA*Sex +
                               (1 | SpeakerID),
                             data = modelData)
summary(AP_VSA_pars)
sjPlot::tab_model(AP_VSA_pars)
```

M0 - Fully unconditional model
```{r}
# Building the model
Int_VSA_m0 <- lmerTest::lmer(Int_M_log ~ 
                               1 +
                               (1 | SpeakerID),
                             data = modelData)
# Model summary
...
```{r}
# Building the model
Int_VSA_m1 <- lmerTest::lmer(Int_M_log ~ 
                               aVSA_log + # newly added variable
                               (1 | SpeakerID),
                             data = modelData)
# Model summary
summary(Int_VSA_m1)
...
# aVSA was a significant predictor and significantly improved the model fit
```

M2 - aVSA + kVSA
```{r}
# Building the model
Int_VSA_m2 <- lmerTest::lmer(Int_M_log ~ 
                               aVSA_log +
                               kVSA_log + # newly added predictor
                               (1 | SpeakerID),
                             data = modelData)
# Model summary
summary(Int_VSA_m2)
...
# Perform likelihood ratio test
# anova(Int_VSA_m1, Int_VSA_m2) # some speakers had missing data for kVSA, resulting in the two models being fitted on different sizes of data. However, kVSA was a significant predictor and was retained in the model.
```

M3 - aVSA + kVSA + Sex
```{r}
# Building the model
Int_VSA_m3 <- lmerTest::lmer(Int_M_log ~ 
                               aVSA_log +
                               kVSA_log +
                               Sex + # newly added predictor
                               (1 | SpeakerID),
                             data = modelData)
# Model summary
summary(Int_VSA_m3)
...
# Perform likelihood ratio test
anova(Int_VSA_m2, Int_VSA_m3)

# Sex was not a significant predictor and did not significantly improve the model fit. However, before removing it, let's test its interactions with the variables.
```
M4 - aVSA*Sex + kVSA*Sex
```{r}
# Building the model
Int_VSA_m4 <- lmerTest::lmer(Int_M_log ~ 
                               aVSA_log*Sex +
                               kVSA_log*Sex +
                               (1 | SpeakerID),
                             data = modelData)
...
# The interactions between Sex and the predictors were not a significant and did not significantly improve the model fit. Therefore, Sex was removed from the model.
```
M5 - aVSA*Sex
```{r}
# Building the model
Int_VSA_m5 <- lmerTest::lmer(Int_M_log ~ 
                               aVSA_log +
                               kVSA_log +
                               (1 | SpeakerID),
                             data = modelData)
...
# Saving the model
base::saveRDS(
  object = Int_VSA_finalModel,
  file = "Models/Int_VSA_finalModel.rds")
...
```{r}
base::saveRDS(Int_VSA_m1, file = "Models/Int_VSA_m1.rds")
base::saveRDS(Int_VSA_m2, file = "Models/Int_VSA_m2.rds")
base::saveRDS(Int_VSA_m3, file = "Models/Int_VSA_m3.rds")
base::saveRDS(Int_VSA_m4, file = "Models/Int_VSA_m4.rds")
base::saveRDS(Int_VSA_m5, file = "Models/Int_VSA_m5.rds")
...
## Acoustic Distance
  hist(modelData$acoDistance) # Looks right-skewed
  performance::check_normality(modelData$acoDistance) # Non-normality detected
  performance::check_distribution(modelData$acoDistance)

## Kinematic Distance
  hist(modelData$kinDistance) # Almost looks normal, but a cluster of values on the lower end
  performance::check_normality(modelData$kinDistance) # Non-normality detected
  performance::check_distribution(modelData$kinDistance)

## F2 Slope
  hist(modelData$F2_Slope) # Slight right skew
  performance::check_normality(modelData$F2_Slope) # Non-normality detected
  performance::check_distribution(modelData$F2_Slope)

# Kinematic Speed
  hist(modelData$kinSpeed) # Slight right skew
  performance::check_normality(modelData$kinSpeed) # Non-normality detected
  performance::check_distribution(modelData$kinSpeed)
...
modelData <- modelData %>%
  dplyr::mutate(Int_M_log = log(Int_M),
                AP_M_log = log(AP_M),
                acoDistance_log = log(acoDistance),
                kinDistance_log = log(kinDistance),
                F2_Slope_log = log(F2_Slope),
                kinSpeed_log = log(kinSpeed + .1) # Applying a constant of .1 because all kinSpeed values are between 0 and 1
                )
...
## Comprehensive Model
```{r}
Int_ai_comp <- robustlmm::rlmer(Int_M ~ 
                                acoDistance*Sex +
                                kinDistance*Sex +
                                F2_Slope*Sex +
                                kinSpeed*Sex +
                             ...
                             data = modelData)
summary(Int_ai_comp)
sjPlot::tab_model(Int_ai_comp)

Int_ai_pars <- robustlmm::rlmer(Int_M ~ 
                                  kinDistance+
                                  Sex +
                                  (1 | SpeakerID),
                             data = modelData)
summary(Int_ai_pars)
sjPlot::tab_model(Int_ai_pars)

# AP
AP_ai_comp <- robustlmm::rlmer(AP_M ~ 
                                acoDistance*Sex +
                                kinDistance*Sex +
                                F2_Slope*Sex +
                                kinSpeed*Sex +
                             ...
                             data = modelData)
summary(AP_ai_comp)
sjPlot::tab_model(AP_ai_comp)

AP_ai_pars <- robustlmm::rlmer(AP_M ~ 
                                  kinDistance+
                                  Sex +
                                  (1 | SpeakerID),
                             data = modelData)
...
```{r}
# Building the model
Int_ai_m0 <- lmerTest::lmer(Int_M_log ~ 1 +
                         (1 | SpeakerID),
                     data = modelData)
# Model summary
summary(Int_ai_m0)
...
```{r}
# Building the model
Int_ai_m1 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log +
                              (1 | SpeakerID),
                     data = modelData)
# Model summary
...
# Perform likelihood ratio test
anova(Int_ai_m0, Int_ai_m1) # Adding acoDistance significantly improved the model fit

```
M2 - acoDistance + F2_Slope
```{r}
# Building the model
Int_ai_m2 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log +
                              F2_Slope_log + # newly added predictor
                              (1 | SpeakerID),
                            data = modelData)
# Model summary
summary(Int_ai_m2)
...
# F2 Slope was not a significant predictor at alpha = 0.0125, and was not retained in the model

# Building the model
Int_ai_m2.5 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log +
                              (1 | SpeakerID),
                            data = modelData)
# Model summary
...
M3 - acoDistance + F2_Slope + kinDistance
```{r}
# Building the model
Int_ai_m3 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log +
                              kinDistance_log + # newly added predictor
                              (1 | SpeakerID),
                            data = modelData)
# Model summary
summary(Int_ai_m3)
...
# Perform likelihood ratio test
#anova(Int_ai_m2.5, Int_ai_m3) Can't compare because some speakers are missing kinDistance values, so the models were not fitted to the same size of the dataset. However, kinDistance was a significant predictor and will be retained in the model.

```
M4 - acoDistance + kinDistance + kinSpeed
```{r}
# Building the model
Int_ai_m4 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log +
                              kinDistance_log +
                              kinSpeed_log + # newly added predictor
                              (1 | SpeakerID),
                            data = modelData)
# Model summary
summary(Int_ai_m4)
...
# kinSpeed was not a significant predictor and did not significantly improve the model fit and was not retained.

# Building the model
Int_ai_m4.5 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log +
                              kinDistance_log +
                              (1 | SpeakerID),
                            data = modelData)
...
summary(Int_ai_m4.5)
```
M5 - acoDistance + kinDistance + kinSpeed + Sex
```{r}
# Building the model
Int_ai_m5 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log +
                              kinDistance_log + 
                              Sex + # newly added variable
                         (1 | SpeakerID),
                     data = modelData)
# Model summary
summary(Int_ai_m5)
...
# Perform likelihood ratio test
anova(Int_ai_m4.5, Int_ai_m5)

# Sex was not a significant predictor. However, before removing it, let's test the interaction between Sex and the predictors.
```
M6 - acoDistance*Sex + kinDistance*Sex
```{r}
# Building the model
Int_ai_m6 <- lmerTest::lmer(Int_M_log ~ 
                              acoDistance_log*Sex +
                              kinDistance_log*Sex +
                         (1 | SpeakerID),
                     data = modelData)
...
# Neither Sex interactions were significant at .0125 and did not significantly improve model fit, and were not retained in the model.

# Building the model
Int_ai_m6.5 <- lmerTest::lmer(Int_M_log ~ 
                                acoDistance_log +
                                kinDistance_log +
                         (1 | SpeakerID),
                     data = modelData)
...
#### Final Model
```{r}
Int_ai_finalModel <- Int_ai_m6.5

summary(Int_ai_finalModel)

# Saving the model
base::saveRDS(
  object = Int_ai_finalModel,
  file = "Models/Int_ai_finalModel.rds")
...
Saving the models for the table
```{r}
base::saveRDS(Int_ai_m0, file = "Models/Int_ai_m0.rds")
base::saveRDS(Int_ai_m1, file = "Models/Int_ai_m1.rds")
base::saveRDS(Int_ai_m2, file = "Models/Int_ai_m2.rds")
base::saveRDS(Int_ai_m2.5, file = "Models/Int_ai_m2_5.rds")
base::saveRDS(Int_ai_m3, file = "Models/Int_ai_m3.rds")
base::saveRDS(Int_ai_m4, file = "Models/Int_ai_m4.rds")
base::saveRDS(Int_ai_m4.5, file = "Models/Int_ai_m4_5.rds")
base::saveRDS(Int_ai_m5, file = "Models/Int_ai_m5.rds")
base::saveRDS(Int_ai_m6, file = "Models/Int_ai_m6.rds")
base::saveRDS(Int_ai_m6.5, file = "Models/Int_ai_m6_5.rds")
...
```{r}
# Building the model
AP_VSA_m0 <- lmerTest::lmer(AP_M_log ~
                              1 +
                              (1 | SpeakerID),
                            data = modelData)
# Model summary
...
```{r}
# Building the model
AP_VSA_m1 <- lmerTest::lmer(AP_M_log ~ 
                              aVSA_log + # newly added variable
                              (1 | SpeakerID),
                            data = modelData)
# Model summary
...
# aVSA was a significant predictor and significantly improved the model fit. Therefore, it was retained in the model.
```

M2 - aVSA + kVSA
```{r}
# Building the model
AP_VSA_m2 <- lmerTest::lmer(AP_M_log ~
                              aVSA_log +
                              kVSA_log + # newly added variable
                              (1 | SpeakerID),
                            data = modelData)
...
# Perform likelihood ratio test
#anova(AP_VSA_m1, AP_VSA_m2) # some speakers had missing data for kVSA, resulting in the two models being fitted on different sizes of data. However, kVSA was a significant predictor and was retained in the model.
```

M3 - aVSA + kVSA + Sex
```{r}
# Building the model
AP_VSA_m3 <- lmerTest::lmer(AP_M_log ~ 
                              aVSA_log +
                              kVSA_log +
                              Sex + # newly added variable
                              (1 | SpeakerID),
...
```{r}
# Building the model
AP_VSA_m4 <- lmerTest::lmer(AP_M_log ~ 
                              aVSA_log*Sex +
                              kVSA_log*Sex +
                              (1 | SpeakerID),
                            data = modelData)
...
```{r}
# Building the model
AP_VSA_m5 <- lmerTest::lmer(AP_M_log ~
                              aVSA_log +
                              kVSA_log +
                              (1 | SpeakerID),
                            data = modelData)
...
# Saving the model
base::saveRDS(
  object = AP_VSA_finalModel,
  file = "Models/AP_VSA_finalModel.rds")
...
Saving the models for the table
```{r}
base::saveRDS(AP_VSA_m1, file = "Models/AP_VSA_m1.rds")
base::saveRDS(AP_VSA_m2, file = "Models/AP_VSA_m2.rds")
base::saveRDS(AP_VSA_m3, file = "Models/AP_VSA_m3.rds")
base::saveRDS(AP_VSA_m4, file = "Models/AP_VSA_m4.rds")
base::saveRDS(AP_VSA_m5, file = "Models/AP_VSA_m5.rds")
...
```{r}
# Building the model
AP_ai_m0 <- lmerTest::lmer(AP_M_log ~
                             1 +
                             (1 | SpeakerID),
                           data = modelData)
# Model summary
...
```{r}
# Building the model
AP_ai_m1 <- lmerTest::lmer(AP_M_log ~
                             acoDistance_log + # newly added variable
                             (1 | SpeakerID),
                           data = modelData)
# Model summary
summary(AP_ai_m1)
...
# acoDistance was a significant predictor in the model and significantly improved the model fit.
```
M2 - acoDistance + F2_Slope
```{r}
# Building the model
AP_ai_m2 <- lmerTest::lmer(AP_M_log ~ 
                             acoDistance_log +
                             F2_Slope_log + # newly added variable
                             (1 | SpeakerID),
                           data = modelData)
# Model summary
summary(AP_ai_m2)
...
# F2 slope was a significant predictor in the model and significantly improved the model fit. Therefore, it was retained in the model.
```

M3 - acoDistance + F2_Slope + kinDistance
```{r}
# Building the model
AP_ai_m3 <- lmerTest::lmer(AP_M_log ~ 
                             acoDistance_log +
                             F2_Slope_log +
                             kinDistance_log +
                             (1 | SpeakerID),
...
# Perform likelihood ratio test
#anova(AP_ai_m2, AP_ai_m3) # Can't compare because some speakers are missing kinDistance values, so the models were not fitted to the same size of the dataset. However, kinDistance was a significant predictor and will be retained in the model.

```
M4 - acoDistance + F2_Slope + kinDistance + kinSpeed
```{r}
# Building the model
AP_ai_m4 <- lmerTest::lmer(AP_M_log ~ 
                             acoDistance_log +
                             F2_Slope_log +
                             kinDistance_log +
                             kinSpeed_log + # newly added predictor
...
# Perform likelihood ratio test
anova(AP_ai_m3, AP_ai_m4) # kinSpeed_log significantly improved the model fit.
performance::check_collinearity(AP_ai_m4) # There are a lot of predictors - let's check for issues of multicolinearity. Looks OK.

# kinSpeed was a significant predictor in the model and significantly improved the model fit. However, kinDistance was no longer significant.

# Building the model
AP_ai_m4.5 <- lmerTest::lmer(AP_M_log ~ 
                             acoDistance_log +
                             F2_Slope_log +
                             kinSpeed_log +
                             (1 | SpeakerID),
...
# Perform likelihood ratio test
anova(AP_ai_m4, AP_ai_m4.5)
```
M5 - acoDistance*Sex + F2_Slope*Sex + kinDistance*Sex + kinSpeed*Sex
```{r}
# Building the model
AP_ai_m5 <- lmerTest::lmer(AP_M_log ~ 
                             acoDistance_log +
                             F2_Slope_log +
                             kinSpeed_log +
                             Sex + # newly added predictor
...
# Perform likelihood ratio test
anova(AP_ai_m4.5, AP_ai_m5)

# Speaker Sex was not a significant predictor, however, it did improve model fit. Before we decide about removing it or retaining it, let's check to see how Sex interacts with the model predictors.

```
M6 - kinDistance*Sex + kinSpeed*Sex
```{r}
# Building the model
AP_ai_m6 <- lmerTest::lmer(AP_M_log ~ 
                             acoDistance_log*Sex + # Interactions added
                             F2_Slope_log*Sex +
                             kinSpeed_log*Sex +
                             (1 | SpeakerID),
                           data = modelData)
...
# Perform likelihood ratio test
anova(AP_ai_m5, AP_ai_m6) # Interactions did not improve model fit

# The sex interactions were not significant and they did not improve model fit. Therefore, we'll remove them an opt for a simpler model.

```
M7 - kinDistance + Sex
```{r}
# Building the model
AP_ai_m7 <- lmerTest::lmer(AP_M_log ~ 
                             acoDistance_log +
                             F2_Slope_log +
                             kinSpeed_log +
                             (1 | SpeakerID),
...
# Saving the model
base::saveRDS(
  object = AP_ai_finalModel,
  file = "Models/AP_ai_finalModel.rds")
...
Saving the models for the table
```{r}
base::saveRDS(AP_ai_m1, file = "Models/AP_ai_m1.rds")
base::saveRDS(AP_ai_m2, file = "Models/AP_ai_m2.rds")
base::saveRDS(AP_ai_m3, file = "Models/AP_ai_m3.rds")
base::saveRDS(AP_ai_m4.5, file = "Models/AP_ai_m4_5.rds")
base::saveRDS(AP_ai_m4, file = "Models/AP_ai_m4.rds")
base::saveRDS(AP_ai_m5, file = "Models/AP_ai_m5.rds")
base::saveRDS(AP_ai_m6, file = "Models/AP_ai_m6.rds")
base::saveRDS(AP_ai_m7, file = "Models/AP_ai_m7.rds")
...
# VSA ----
## Fit model 1
model1 <- base::readRDS(file = "Models/Int_VSA_finalModel.rds")
## Fit model 2
model2 <- base::readRDS(file = "Models/AP_VSA_finalModel.rds")

## Compare Marginal R-squared values
r_squared1 <- MuMIn::r.squaredGLMM(model1)[1]
r_squared2 <- MuMIn::r.squaredGLMM(model2)[1]

## Compare the Marginal R-squared values
cat("Marginal R-squared for model 1:", r_squared1, "\n")
cat("Marginal R-squared for model 2:", r_squared2, "\n")

## Compare Conditional R-squared values
r_squared1 <- MuMIn::r.squaredGLMM(model1)[2]
r_squared2 <- MuMIn::r.squaredGLMM(model2)[2]

## Compare the ConditionalR-squared values
cat("Conditional R-squared for model 1:", r_squared1, "\n")
cat("Conditional R-squared for model 2:", r_squared2, "\n")

# kinDistance ----
library(MuMIn) # install.packages('MuMIn')

## Fit model 1
model1 <- base::readRDS(file = "Models/Int_ai_finalModel.rds")
## Fit model 2
model2 <- base::readRDS(file = "Models/AP_ai_finalModel.rds")
...
model_Int <- base::readRDS(file = "Models/Int_VSA_finalModel.rds")
model_AP <- base::readRDS(file = "Models/AP_VSA_finalModel.rds")
  modelData <- rbind(model_Int@frame %>%
                      dplyr::mutate(fit = predict(model_Int),
                                    ratingType = "Int") %>%
                      dplyr::rename(Rating = Int_M),
                    model_AP@frame %>%
                      dplyr::mutate(fit = predict(model_AP),
                                    ratingType = "AP") %>%
...
# Scatter plot + Model Predictions  ----
## Get the model data
model_Int <- base::readRDS(file = "Models/Int_ai_finalModel.rds")
model_AP <- base::readRDS(file = "Models/AP_ai_finalModel.rds")

  modelData <- dplyr::bind_rows(
    # Intelligibility Model Data
    model_Int@frame %>%
      dplyr::mutate(fit = predict(model_Int),
                    ratingType = "Int") %>%
      dplyr::rename(Rating = Int_M),
    
    # Articulatory Precision Model Data 
    model_AP@frame %>%
      dplyr::mutate(fit = predict(model_AP),
                    ratingType = "AP") %>%
      dplyr::rename(Rating = AP_M)) %>%
...
ggplot(plot_ai_data, aes(x = kinDistance,
                         y = Rating,
                         color = Group,
                         group = StudyID)) +
  geom_point(alpha = 0.3) +
  geom_line(data = modelData, aes(y = fit,
                                  group = interaction(StudyID, kinSpeed),
                                  color = Group),
            alpha = 0.5) +
  facet_grid(ratingType ~ Sex) +
  theme_minimal() +
  labs(x = "Kinematic (TB) Distance (mm^2)",
       y = "Articulatory Precision (VAS)",
       color = "Group")

sjPlot::plot_model(model_AP, type = "pred")
sjPlot::tab_model(model_AP)

ggeffects::ggpredict(
  model = model_AP,
  terms = c("kinDistance", "kinSpeed", "Sex"),
  type = "random"
  #condition = c(kinSpeed = 0.0801218)
)

plot_ai_data %>%
  dplyr::filter(ratingType == "Intelligibility (VAS)") %>%
  dplyr::summarise(M = mean(kinSpeed, na.rm = T))
